...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, \
    ...
    if (frame % P.STEER_STEP) == 0:
      # Disable LKAs if blinkers on
      lkas_enabled = enabled and not CS.out.steerWarning and CS.out.vEgo > P.MIN_STEER_SPEED and not CS.out.leftBlinker and not CS.out.rightBlinker
      if lkas_enabled:
    ...
    if (frame % 4) == 0:
      ...
      standstill = CS.pcm_acc_status == AccState.STANDSTILL  # Ignore CS.out.standstill (velocity) and PCM state instead
      ...
      acc_enabled = enabled
      # Auto-resume if PCM not in standstill and OP requests gas
    ...
    if (frame % 4) == 0:
      can_sends.append(gmcan.create_acc_dashboard_command(self.packer_pt, CanBus.POWERTRAIN, enabled, hud_v_cruise * CV.MS_TO_KPH, hud_show_car, CS.get_follow_level()))
